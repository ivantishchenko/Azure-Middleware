...
class ExperimentPlotter:
    ...
    def plot_baseline_aggregate(self, filename1, filename2):
            ...
            for virtual_client in range(self.CLIENTS_RANGE_BEG, self.CLIENTS_RANGE_END + 1, self.CLIENTS_RANGE_STEP):
                ...
                for repetition in range(1, self.REP_NUMBER + 1):
                    ...
                    for machine in range(1, MACHINES_RANGE + 1):
                        logfile_name = self.LOGFILES_PATH + "/" + str(worker) + "/multi_get_{}_{}_{}.log".format(virtual_client, repetition, machine)
                        print(logfile_name)
        ...
        legends = []
        legends_name = []
        for i in range(len(self.KEYS_RANGE)):
            T = T_workers[i]
            ...
            legends.append(p)
            legends_name.append("Keys # " + str(self.KEYS_RANGE[i]))
