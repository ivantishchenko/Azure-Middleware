...
for i in range(MACHINES_NUMBER):
    ...
    plt.errorbar(clients, OUT_T, yerr=OUT_STD, fmt='-o', ecolor='r', color=colors[i % MACHINES_NUMBER], label="Client machine " + str(i+1))
...
for i in range(MACHINES_NUMBER):
    ...
    plt.errorbar(clients, OUT_R, yerr=OUT_STD, fmt='-o', ecolor='r', color=colors[i % MACHINES_NUMBER], label="Client machine " + str(i+1))
