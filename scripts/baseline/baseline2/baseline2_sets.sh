...
# Write only workload
cmdpart="memtier_benchmark-master/memtier_benchmark --protocol=memcache_text --ratio=1:0 --expiry-range=9999-10000 --key-maximum=10000 --data-size=1024 --hide-histogram"
...
LOG_FILE_DIR="logfiles_baseline_2_servers_SET"
...
do
	...
	do
		...
		cmd1="${cmdpart} --server=${server1} --port=${port1} --test-time=${time} --clients=${c} --threads=${threads} --out-file=${LOG_FILE_DIR}/baseline_${c}_${rep}_1.log"
		cmd2="${cmdpart} --server=${server2} --port=${port2} --test-time=${time} --clients=${c} --threads=${threads} --out-file=${LOG_FILE_DIR}/baseline_${c}_${rep}_2.log"
