...
do
    ...
    do
        for rep in `seq 1 3`;
        do
            cmd1="${cmdpart_SET} --server=${middleware1_privat} --port=${mw_port} --test-time=${time} --clients=${c} --threads=${threads_single} --out-file=${LOG_FILE_DIR_SINGLE_SET_CLIENT}/${w}/baselineMW_${c}_${rep}_1.logs"
            cmd_mw="${CMD_PART_MW1} -t ${w} > ${LOG_FILE_DIR_SINGLE_SET_MW}/${w}/baselineMW_${c}_${rep}_1.log &"
